...
PlotSac() {
    Exepssac=/home/tianye/usr/bin/pssac
    ...
    $Exepssac $REG $SCA -Ba${xmrk}f${xtic}:"":/:$label:wESn -O -K -X${Xoffset} -Y${Yoffset} $sacfp >> $psout
    #gmtset LABEL_OFFSET $los
    ### label the sac info ###
    t_info=`saclst KEVNM KSTNM DIST USER0 f $sacfp | awk '{print $2" - "$3"   Distance="$4"   TimeLength="$5"days"}'`
    echo 0 11.7 $t_info | pstext -R0/10/0/10 -WO3/70/70/70 -J -O -N -P -K >> $psout
    rm $sacfp
}
...
PlotFTAN() {
    ...
    grdimage -P $REG -JX${SCAx}/${SCAy} ${dirpair}/${sacf}_amp.grd \
        -C${dirpair}'/FTAN.cpt' -X${Xoffset} -Y${Yoffset} -K > $psout
    #grdimage $REG -JX${SCAx}/${SCAy} ${dirpair}/amp.grd \
    #    -C${dirpair}'/FTAN.cpt' -X${Xoffset} -Y${Yoffset} -K > $psout
    REG=-R${perl}/${perh}/0.2/4.5
    psbasemap $REG -JX${SCAx}l/${SCAy} -Ba3f3:"period (sec)":/a1.0f0.5:"velocity (km/s)"::."":WSen -V -O -K >> $psout
...
PlotSNR() {
    ...
    pslegend -JX2.6i/4.1i -R -Dx.1i/3.5i+w1.i+jBL -F+pblack -t10 -O -K <<- EOF >> $psout
...
sacf=$1
Initialize ${sacf}
RunFTAN ${sacf} '-1 0.25 4.5 0.8 40. 30. 2.0 3. 0.99 10. 15. 1.5 3.0 0.6 3.' 0.08
FilterSac ${sacf}
PlotFTAN ${sacf} 1.7 5.
PlotSac ${sacf} 0. 8.8
# PlotSNR ${sacf} 12.3 -8.5

#for sacf in `ls COR*_*.SAC`; do
#    Initialize ${sacf}
#    RunFTAN ${sacf} '-1 0.15 4.5 0.8 30. 30. 2.0 3. 0.99 10. 15. 1.5 3.0 0.6 3.' 0.08
#    FilterSac ${sacf}
#    PlotFTAN ${sacf} 2. 2.
#    PlotSac ${sacf} 0. 8.8
#    PlotSNR ${sacf} 12.3 -8.5
#    ps2eps $psout
#    epsfile=`echo $psout | awk -F'.ps' '{print $1".eps"}'`
#    epstopdf $epsfile
#done 
